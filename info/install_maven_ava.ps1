...
$installDir = "$env:USERPROFILE\apache-maven-$mavenVersion"  # Install in user directory
...
if (-not $currentPath.Contains($mavenBinPath)) {
    [System.Environment]::SetEnvironmentVariable("PATH", "$currentPath;$mavenBinPath", [System.EnvironmentVariableTarget]::User)
